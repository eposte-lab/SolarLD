...
async def evaluate_cluster_ab_tests(sb: Any) -> dict[str, Any]:
    ...
    pairs: set[tuple[str, str]] = {
        (r["tenant_id"], r["cluster_signature"]) for r in active_resp.data
    }
...
async def _generate_new_round(
    ...
    await persist_variant_pair(sb, tenant_id, cluster_sig, new_round, va, vb)
...
async def manually_promote_variant(
    ...
    previous_winner = {
        k: winner[k]
        for k in ("copy_subject", "copy_opening_line", "copy_proposition_line", "cta_primary_label")
        if k in winner
    }
    await _generate_new_round(sb, tenant_id, cluster_sig, round_number + 1, previous_winner)
