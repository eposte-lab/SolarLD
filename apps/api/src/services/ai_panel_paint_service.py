...
def build_paint_prompt(*, panel_count: int, kwp: float | None = None) -> str:
    ...
    return (
        ...
        "STEP 2 — PRESERVE THE EXISTING ROOF (CRITICAL). The roof's "
        "existing three-dimensional shape MUST stay exactly as in the "
        "input: every pitch and slope angle, every ridge line, hip, "
        "valley and eave, the roof covering itself (clay/terracotta "
        "tiles, shingles, sheet metal or flat membrane) and every "
        "rooftop feature (chimneys, skylights, dormers, vents). "
        "ABSOLUTELY DO NOT flatten a pitched or tiled roof. DO NOT "
        "remove, hide or replace the roof tiles. DO NOT turn a sloped "
        "roof into a flat slab or terrace. DO NOT change the roof's "
        "outline or height. The pitched tiled roof must still be a "
        "pitched tiled roof in the output — only with panels added on "
        "its surface. "
        # ── 3. Place the panels ──────────────────────────────────────
