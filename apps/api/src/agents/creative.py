...
Pipeline (rendering-v2):
    ...
    ai_panel_paint_service.generate_after_with_panels(before_url)
          → nano-banana identifies the rooftop, paints panels on it,
            framing locked to the before
          → AFTER frame: the painted photo, used directly — the
            before→after transition is a plain crossfade of the two
        ↓
    upload after.png to Supabase Storage
        ↓
    compute_roi(kwp, yearly_kwh, subject_type)  → leads.roi_data
        ↓
    video-renderer sidecar: render_transition(before_url, after_url, roi)
          → FFmpeg crossfade wipe reveals the panels gradually between
            the two aligned frames, ROI stats overlaid at the end
    ...
    async def execute(self, payload: CreativeInput) -> CreativeOutput:
        ...
        if lat is None or lng is None:
            ...
            try:
                ...
                # 4a) BEFORE image — high-res aerial crop, no panels.
                ...
                # aerial crop that nano-banana paints panels onto. Both
                # source a high-res Mapbox satellite tile for the crop.
                ...
                # Il `before.png` resta volutamente "nudo": niente strip
                # bakeata. La fascia navy con "Risparmio annuo stimato"
                # vive solo sull'after, così la crossfade wipedown la
                # rivela gradualmente durante la transizione invece di
                # mostrarla già da subito sul frame iniziale.
                ...
                _log_api_cost(
                    ...
                    endpoint="solar/buildingInsights",
                    cost_cents=2,  # ~$0.02 — imagery now comes from Mapbox
                    status="success",
                    metadata={
                        "lead_id": payload.lead_id,
                        "panels": len(insight.panels),
                ...
                # 4b) AFTER image — nano-banana paints photoreal panels on
                # the roof. The painted frame IS the after; the
                ...
                if settings.creative_skip_replicate and after_bytes_offline is not None:
                    ...
                    # at exact Solar lat/lng — deterministic and zero-cost.
