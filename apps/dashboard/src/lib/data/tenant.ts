...
export const getCurrentTenantContext = cache(async (): Promise<TenantContext | null> => {
  ...
  const { data: member } = await supabase
    .from('tenant_members')
    .select('tenant_id, role, tenants:tenants(id, business_name, brand_primary_color, brand_logo_url, contact_email, whatsapp_number, email_from_domain, email_from_name, email_from_domain_verified_at, tier, settings, demo_device_limit_enabled, demo_device_max_total, demo_device_idle_timeout_minutes)')
    .eq('user_id', user.id)
    .limit(1)
    .maybeSingle();
