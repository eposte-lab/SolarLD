...
export interface PortalSessionStats {
  /** Number of distinct sessions ever recorded. */
  sessions: number;
  /** Total time spent on the portal across all sessions (seconds). */
  total_time_sec: number;
  /** Deepest scroll percentage reached (0-100). */
  deepest_scroll_pct: number;
  /** ISO timestamp of the most recent event, or null. */
  last_event_at: string | null;
  /** True if the lead appears to be browsing right now (event in last 2 min). */
  is_live_now: boolean;
}
...
export async function getPortalSessionStats(
  ...
  for (const row of rows) {
    ...
    // Scroll milestones imply at least that percentage.
    if (r.event_kind === 'portal.scroll_90') maxScroll = Math.max(maxScroll, 90);
    else if (r.event_kind === 'portal.scroll_50')
      maxScroll = Math.max(maxScroll, Math.max(50, maxScroll));
